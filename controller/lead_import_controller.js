            ...
            worksheet.eachRow(async (row, rowNumber) => {
                if (rowNumber > 1) {
                    ...
                    if (seenEntries.has(entryKey)) {
                        ...
                        invalidRows.push({
                            rowNumber,
                            errors: [`Duplicate entry detected for email '${email}' or phone '${phone}' in the file`],
                            rowData: {
                    ...
                    if (errors.length > 0) {
                        ...
                        if (emailExists || phoneExists) {
                            invalidRows.push({
                                rowNumber,
                                errors: [
                                    emailExists ? `Email '${email}' already exists in the database` : null,
                                    phoneExists ? `Phone number '${phone}' already exists in the database` : null
                                ].filter(Boolean),
                                rowData
                            });
                        } else {
