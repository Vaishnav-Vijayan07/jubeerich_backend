...
exports.saveEmploymentHistory = async (req, res, next) => {
  try {
    ...
    if (existData) {
      ...
      handleFileUpdates(existData, { visaPage, permitCard, salaryAccountStatement, supportingDocuments });
    } else {
...
const handleFileUpdates = (existData, newFiles) => {
  ...
  const { visaPage, permitCard, salaryAccountStatement, supportingDocuments } = newFiles;
